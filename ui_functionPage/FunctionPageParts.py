...
from ui_functionPage.MyFileFunction import copy_folder, copy_file, extract_zip, addPACKAGE
from ui_functionPage.MyWidgets import FileDropEdit, DraggableTreeView, DropTargetTreeView, HtmlWidget, SelectTypeComboBox
from ui_functionPage.React import interact
...
class WorkDirectoryArea(QWidget):
    def __init__(self):
        ...
        # 选择文件类型下拉菜单
        select_combo_box_container = QHBoxLayout()
        self.select_type_label = QLabel("选择语言: ")
        self.combo_box = SelectTypeComboBox()
        select_combo_box_container.addWidget(self.select_type_label)
        select_combo_box_container.addWidget(self.combo_box)
        select_combo_box_container.setStretchFactor(self.select_type_label, 1)
        select_combo_box_container.setStretchFactor(self.combo_box, 10)
        ...
        # 开始布局
        self.container = QVBoxLayout()
        self.container.addLayout(select_combo_box_container)
...
class FunctionPage(QWidget):
    def __init__(self):
        ...
        # 按钮逻辑
        # 选择文件类型
        self.work_directory_area.combo_box.currentIndexChanged.connect(self.on_combobox_changed)
        # 设置工作区文件夹
    ...
    def on_combobox_changed(self):
        file_type = self.work_directory_area.combo_box.currentData()
        self.work_directory_area.working_area.setFileType(file_type)
    ...
    def check(self):
        print(f"查重文件类型: {self.work_directory_area.combo_box.currentData()}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            addPACKAGE(temp_folder, self.user, self.work_directory_area.combo_box.currentData())
            ...
            for file_path in selected_files:
                if os.path.isdir(file_path):
                    copy_folder(file_path, temp_folder_code, self.work_directory_area.combo_box.currentData())
